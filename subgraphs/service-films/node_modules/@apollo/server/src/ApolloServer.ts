...
import { newCachePolicy } from '@apollo/server/src/cachePolicy.jsrc/cachePolicy.js';
import { determineApol@apollo/server/src/determineApolloConfig.jserver/src/determineApolloConfig.js';
import {
  ensureError,
  ensureGraphQLError,
  @apollo/server/src/errorNormalize.jsrrors,
} from '@apollo/server/src/errorNormalize.js';
import {
  ApolloServerErr@apollo/server/src/errors/index.jserverValidationErrorCode,
} from '@apollo/server/src/errors/index.js';
import type {
  ApolloServerPlugin,
  BaseContext,
  GraphQLRequest,
...
@apollo/server/src/externalTypes/index.jsLRequestContext,
  HTTPGraphQLHead,
} from '@apollo@apollo/server/src/httpBatching.jsnalTypes/index.js';
import { runPotentiallyBatchedHttpQuery @apollo/server/src/internalPlugin.jsver/src/httpBatching.js';
import { type InternalPluginId, pluginIsInternal } f@apollo/server/src/preventCsrf.jsver/src/internalPlugin.js';
import {
  preventCsrf,
  recomm@apollo/server/src/requestPipeline.jsequestHeaders,
} from '@apollo/server/src/preventCsrf.js';
i@apollo/server/src/runHttpQuery.js_PREFIX, processGraphQLRequest } f@apollo/server/src/utils/schemaManager.jsrequestPipeline.js';
import { @apollo/server/src/utils/isDefined.jsprettyJSONStringify } from '@apollo/serve@apollo/server/src/utils/UnreachableCaseError.js{ SchemaManager } from '@apollo/server/src@apollo/server/src/utils/computeCoreSchemaHash.js { isDefined } from '@apollo/server/src/utils/isDefined.js';
import { UnreachableCaseError } from '@apollo/server/src/utils/U@apollo/server/src/externalTypes/constructor.jst { computeCoreSchemaHash } from '@apollo/server/src/utils/computeCoreSchemaHash.js';
import type { WithRequired } from '@apollo/utils.withrequir@apollo/server/src/incrementalDeliveryPolyfill.jsptionsWithStaticSchema } from @apollo/server/src/utils/HeaderMap.jsexternalTypes/constructor.js';
import type { GatewayExecutor } from '@@apollo/server/src/externalTypes/graphql.jsrface';
...
export class ApolloServer<in out TContext extends BaseContext = BaseContext> {
  ...
  private async addDefaultPlugins() {
    ...
    for (const p of plugins) {
      ...
      @apollo/server/src/plugin/cacheControl/index.jsr your use case.`,
          );
        }
      }
    }
    ...
    // Special case: cache control is on unless you explicitly disable it.
    {
      if (!alreadyHavePluginWithInternalId('CacheControl')) {
        const { ApolloServerPluginCacheControl } = await import(
          '@apollo/server/src/plugin/cacheControl/index.js'
        );
    ...
    // configure an API key.
    {
      ...
      if (!alreadyHavePlugin && apolloConfig.key) {
        if (apolloConfig.graphRef) {
          // Keep this plu@apollo/server/src/plugin/usageReporting/index.js. (Unfortunately despite
          ...
          const { ApolloServerPluginUsageReporting } = await import(
            '@apollo/server/src/plugin/usageReporting/index.js'
          );
          plugins.unshift(
    ...
    @apollo/server/src/plugin/schemaReporting/index.js
    ...
    // Special case: inline tracing is on by default for federated schemas.
    {
     ...
     @apollo/server/src/plugin/inlineTrace/index.jseTrace, we set up inline tracing in "only if
        ...
        const { ApolloServerPluginInlineTrace } = await import(
          '@apollo/server/src/plugin/inlineTrace/index.js'
        );
        plugins.push(
    ...
    // now@apollo/server/src/plugin/landingPage/default/index.jsandingPage because we haven't run
    // serverWillStart yet.)
    ...
    if (!alreadyHavePlugin) {
      ...
      } = await import('@apollo/server/src/plugin/landingPage/default/index.js');
